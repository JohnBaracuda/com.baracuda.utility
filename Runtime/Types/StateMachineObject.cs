...
{
    public class StateMachineObject<TState> : IDisposable where TState : unmanaged, Enum
    {
        #region Fields
        ...
        /// <summary>
        ///     Update the state machine. This method must be called in regular intervals
        /// </summary>
        [PublicAPI]
        public void Update(float deltaTime)
        {
            UpdateStateMachineInternal(deltaTime);
        }
        ...
        #region Public API: Removing Callbacks

        /// <summary>
        ///     Removes a state enter callback for a specific state.
        /// </summary>
        [PublicAPI]
        public void RemoveStateEnterCallback(TState state, Action callback)
        {
            RemoveStateEnterCallbackInternal(state, callback);
        }

        /// <summary>
        ///     Removes a state enter callback for any state.
        /// </summary>
        [PublicAPI]
        public void RemoveStateEnterCallback(Action<TState> callback)
        {
            RemoveStateEnterCallbackInternal(callback);
        }

        /// <summary>
        ///     Removes a state transition callback between two specific states.
        /// </summary>
        [PublicAPI]
        public void RemoveStateTransitionCallback(TState from, TState to, Action callback)
        {
            RemoveStateTransitionCallbackInternal(from, to, callback);
        }

        /// <summary>
        ///     Removes a state exit callback for a specific state.
        /// </summary>
        [PublicAPI]
        public void RemoveStateExitCallback(TState state, Action callback)
        {
            RemoveStateExitCallbackInternal(state, callback);
        }

        /// <summary>
        ///     Removes a state exit callback for any state.
        /// </summary>
        [PublicAPI]
        public void RemoveStateExitCallback(Action<TState> callback)
        {
            RemoveStateExitCallbackInternal(callback);
        }

        /// <summary>
        ///     Removes a state update callback with delta time for a specific state.
        /// </summary>
        [PublicAPI]
        public void RemoveStateUpdateCallback(TState state, Action<float> callback)
        {
            RemoveStateUpdateCallbackInternal(state, callback);
        }

        /// <summary>
        ///     Removes a state update callback for a specific state.
        /// </summary>
        [PublicAPI]
        public void RemoveStateUpdateCallback(TState state, Action callback)
        {
            RemoveStateUpdateCallbackInternal(state, callback);
        }

        /// <summary>
        ///     Removes a state transition update callback for a specific state.
        /// </summary>
        [PublicAPI]
        public void RemoveStateTransitionUpdateCallback(TState to, Action<float> callback)
        {
            RemoveStateTransitionUpdateCallbackInternal(to, callback);
        }

        /// <summary>
        ///     Removes a global state transition callback for any transition.
        /// </summary>
        [PublicAPI]
        public void RemoveStateTransitionCallback(Action<TState, TState> callback)
        {
            RemoveStateTransitionCallbackInternal(callback);
        }

        #endregion


        #region Public API: Removing Transitions

        /// <summary>
        ///     Removes a conditional state transition between two states.
        /// </summary>
        [PublicAPI]
        public void RemoveStateTransition(TState from, TState to)
        {
            RemoveStateTransitionInternal(from, to);
        }

        /// <summary>
        ///     Removes a timed state transition for a specific state.
        /// </summary>
        [PublicAPI]
        public void RemoveTimedStateTransition(TState from)
        {
            RemoveTimedStateTransitionInternal(from);
        }
        ...
        public void Dispose()
        {
            // Clear all state enter callbacks
            _stateEnterCallbacks.Clear();
            _parameterizedStateEnterCallbacks.Clear();
            _stateEnterFromToCallbacks.Clear();

            // Clear all state exit callbacks
            _stateExitCallbacks.Clear();
            _parameterizedStateExitCallbacks.Clear();

            // Clear all state update callbacks
            _stateUpdateCallbacksWithDeltaTime.Clear();
            _stateUpdateCallbacks.Clear();

            // Clear all state transition update callbacks
            _stateTransitionUpdateCallbacks.Clear();
            _globalStateTransitionCallbacks.Clear();

            // Clear all conditional and timed state transitions
            _conditionalStateTransitions.Clear();
            _timedStateTransitions.Clear();

            // Clear all state blockers
            _stateBlocker.Clear();
            _stateMachineBlocker.Clear();

            // Clear any state overrides
            _overrideObjects.Clear();
            ...
            Debug.Log(_log, "State machine disposed and all callbacks and collections cleared.", _color);
        }
        ...
        #region Callback Removing

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void RemoveStateEnterCallbackInternal(TState state, Action callback)
        {
            if (_stateEnterCallbacks.TryGetValue(state, out var list))
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void RemoveStateEnterCallbackInternal(Action<TState> callback)
        {
            _parameterizedStateEnterCallbacks.Remove(callback);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void RemoveStateTransitionCallbackInternal(TState from, TState to, Action callback)
        {
            var transitionKey = Combine(from, to);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void RemoveStateExitCallbackInternal(TState state, Action callback)
        {
            if (_stateExitCallbacks.TryGetValue(state, out var list))
            {
                list.Remove(callback);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void RemoveStateExitCallbackInternal(Action<TState> callback)
        {
            _parameterizedStateExitCallbacks.Remove(callback);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void RemoveStateUpdateCallbackInternal(TState state, Action<float> callback)
        {
            if (_stateUpdateCallbacksWithDeltaTime.TryGetValue(state, out var list))
            {
                list.Remove(callback);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void RemoveStateUpdateCallbackInternal(TState state, Action callback)
        {
            if (_stateUpdateCallbacks.TryGetValue(state, out var list))
            {
                list.Remove(callback);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void RemoveStateTransitionUpdateCallbackInternal(TState to, Action<float> callback)
        {
            if (_stateTransitionUpdateCallbacks.TryGetValue(to, out var list))
            {
                list.Remove(callback);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void RemoveStateTransitionCallbackInternal(Action<TState, TState> callback)
        {
            _globalStateTransitionCallbacks.Remove(callback);
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void UpdateStateMachineInternal(float deltaTime)
        {
#if UNITY_EDITOR
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void RemoveStateTransitionInternal(TState from, TState to)
        {
            if (_conditionalStateTransitions.TryGetValue(from, out var list))
            {
                list.RemoveAll(t => _comparer.Equals(t.ToState, to));
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void RemoveTimedStateTransitionInternal(TState from)
        {
            _timedStateTransitions.Remove(from);
        }
